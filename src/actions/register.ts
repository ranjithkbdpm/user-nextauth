...
export const register = async (values: z.infer<typeof registerSchema>): Promise<{ message: string; success: boolean; user?: UserProps } | { error: string }> => {
    // check the validatedValues of the incoming values
    const validatedValues = registerSchema.safeParse(values);
    // console.log(validatedValues.data);
    // check validation
    if (!validatedValues.success){
        return { success: false, error: "Invalid login values" };
    }
